...
// TODO: optimize
int listen_to_browser(http_context *context) {
    int ret;
    ...
    context->browser_addr_len = sizeof(context->browser_addr);
    context->browser_socket = accept(cache_socket, (struct sockaddr*)&context->browser_addr, &context->browser_addr_len);
    ret = recv(context->browser_socket, context->browser_buf, 1024, 0);
    context->browser_buf[ret] = 0;

    closesocket(cache_socket);
    if (ret <= 0) return RECV_EMPTY;
    return SUCCESS;
}

int get_web_page(http_context *context) {
    int ret;
    ...
    if (ret != 1)
        printf("IP address change error: %d\n", WSAGetLastError());

    /* send */
    connect(server_socket, (struct sockaddr*)&server_addr, sizeof(server_addr));
    ret = send(server_socket, context->browser_buf, 1024, 0);
    while(1) {
        ret = recv(server_socket, server_buf, 16348, 0);
        if (ret <= 0) break;
        server_buf[ret] = 0;
#ifdef DEBUG
        printf("recv:%d bytes\n", ret);
#endif
        send(context->browser_socket, server_buf, ret, 0);
    }

    /* close sockets */
    closesocket(context->browser_socket);
